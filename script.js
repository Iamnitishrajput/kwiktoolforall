...
function openUtility(id){const u={merge:['PDF TOOL','Merge PDF','Combine multiple PDF files into one PDF locally.'],split:['PDF TOOL','Split PDF','Extract selected pages or ranges into a new PDF locally.'],'compress-pdf':['PDF TOOL','Compress PDF','Choose how much to reduce the PDF while keeping useful quality.'],'resize-image':['IMAGE TOOL','Resize Image','Change image dimensions without uploading your file.'],'compress-image':['IMAGE TOOL','Compress Image','Reduce image size with a quality level you control.']}[id];if(!u)return;window.__utilityId=id;resetSuccess();[workspaceHead,upload,fileToolbar,pagesSection,footer].forEach(x=>x&&x.classList.add('tool-hidden'));$('utilityWorkspace').hidden=false;$('utilityKicker').textContent=u[0];$('utilityTitle').textContent=u[1];$('utilityDesc').textContent=u[2];$('utilityStatus').textContent='Ready.';openToolModal();const body=$('utilityBody');({merge:renderMergeTool,split:renderSplitTool,'compress-pdf':renderCompressPdfTool,'resize-image':renderResizeTool,'compress-image':renderCompressTool}[id])(body)}
function closeUtility(){if($('utilityWorkspace'))$('utilityWorkspace').hidden=true;resetSuccess();closeImagePdf()}
