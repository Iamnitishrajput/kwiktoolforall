...
function openSelectedTool(id){const t=tools.find(x=>x.id===id);if(!t)return;if(id==='image-pdf'){openImagePdf();return}search.value=t.name;render(t.name);$('tools').scrollIntoView({behavior:'smooth',block:'start'});showToast(`${t.name} is selected. Its processing engine is coming next.`)}
...
const _oldOpenTool=openTool;openTool=function(id){if(id==='scanner'){openScanner();return}_oldOpenTool(id)};
