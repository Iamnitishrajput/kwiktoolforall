...
function renderCompressPdfTool(body){
  ...
  body.querySelectorAll('[data-pdf-level]').forEach(b=>b.onclick=()=>{level=b.dataset.pdfLevel;body.querySelectorAll('[data-pdf-level]').forEach(x=>x.classList.toggle('active',x===b));const cfg=levelCfg[level];dpi.value=String(cfg.dpi);quality.value=Math.round(cfg.q*100);qualityValue.textContent=quality.value+'%';if(file)update(`${labelFor(level)} selected · ${formatBytes(file.size)} original.`)});
  target.onchange=()=>{customWrap.hidden=target.value!=='custom'};
  quality.oninput=()=>qualityValue.textContent=quality.value+'%';
  run.onclick=async()=>{if(!file||!originalPdf)return;run.disabled=true;run.textContent='Compressing…';$('compressResult').hidden=true;try{const targetBytes=target.value==='custom'?Math.max(1,Number(customTarget.value)*1048576):target.value==='none'?0:Number(target.value);let q=Math.max(.35,Math.min(.95,Number(quality.value)/100)),d=Number(dpi.value),best=null;const attempts=targetBytes?[{q,d},{q:Math.max(.3,q-.12),d},{q:Math.max(.25,q-.22),d},{q:Math.max(.25,q-.3),d:Math.max(60,d-18)},{q:.25,d:Math.max(48,d-30)},{q:.22,d:48}]:[{q,d}];for(let i=0;i<attempts.length;i++){const a=attempts[i],blobs=await renderPdfToJpegs(originalPdf,a.q,a.d,gray.checked,update),out=await jpegBlobsToPdf(blobs,originalPdf);best={blob:out,q:a.q,d:a.d};if(!targetBytes||out.size<=targetBytes)break}downloadBlob(best.blob,'KwikToolForAll_Compressed.pdf');const saved=Math.max(0,file.size-best.blob.size),pct=file.size?saved/file.size*100:0;$('compressResult').innerHTML=`<div class="compress-result-card"><div><span>Original</span><strong>${formatBytes(file.size)}</strong></div><div class="result-arrow">→</div><div><span>Compressed</span><strong>${formatBytes(best.blob.size)}</strong></div><div class="result-saving"><strong>${pct>0?pct.toFixed(1):'0'}%</strong><span>${saved>0?'smaller':'size change'}</span></div></div><small>${best.blob.size<=file.size?'Compression complete.':'This PDF was already highly optimized; the output could not be made smaller without further quality loss.'}</small>`;$('compressResult').hidden=false;showCompressPdfSuccess(file.size,best.blob.size,pct)}catch(e){console.error(e);showToast('Could not compress this PDF. Try a different compression level.');update('Compression failed.')}finally{run.disabled=!file;run.textContent='Compress PDF →'}};
}
