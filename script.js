...
function detectDocumentCrop(im){
  const w=Math.min(700,im.naturalWidth),h=Math.min(700,im.naturalHeight),c=document.createElement('canvas');c.width=w;c.height=h;const x=c.getContext('2d');x.drawImage(im,0,0,w,h);const d=x.getImageData(0,0,w,h).data;
  const border=[];for(let i=0;i<w;i++){let a=i*4,b=((h-1)*w+i)*4;border.push((d[a]+d[a+1]+d[a+2])/3,(d[b]+d[b+1]+d[b+2])/3)}for(let y=1;y<h-1;y++){let a=(y*w)*4,b=(y*w+w-1)*4;border.push((d[a]+d[a+1]+d[a+2])/3,(d[b]+d[b+1]+d[b+2])/3)}
  const avg=border.reduce((a,b)=>a+b,0)/border.length;let minX=w,minY=h,maxX=0,maxY=0,found=0,thr=Math.max(18,Math.min(55,Math.abs(avg-128)*.2+24));
  ...
  if(found<(w*h*.02)||maxX-minX<w*.45||maxY-minY<h*.45)return{x:0,y:0,w:1,h:1};
...
function enableScannerCrop(mode){const p=currentScan(),canvas=$('scannerCanvas');if(!p||!canvas)return;scannerCropMode=mode;showToast(mode==='square'?'Drag to select a square crop.':'Drag to select the crop area.');let start=null;
  const finish=e=>{if(!start)return;const box=canvas.getBoundingClientRect();let x1=(start.x-box.left)/box.width,y1=(start.y-box.top)/box.height,x2=(e.clientX-box.left)/box.width,y2=(e.clientY-box.top)/box.height;x1=Math.max(0,Math.min(1,x1));x2=Math.max(0,Math.min(1,x2));y1=Math.max(0,Math.min(1,y1));y2=Math.max(0,Math.min(1,y2));let x=Math.min(x1,x2),y=Math.min(y1,y2),w=Math.abs(x2-x1),h=Math.abs(y2-y1);if(mode==='square'){const side=Math.min(w,h);if(x2<x1)x=x1-side;if(y2<y1)y=y1-side;w=h=side;x=Math.max(0,Math.min(x,1-side));y=Math.max(0,Math.min(y,1-side))}if(w>.03&&h>.03)p.crop={x,y,w,h};scannerCropMode=null;canvas.style.cursor='default';canvas.removeEventListener('pointerdown',down);window.removeEventListener('pointerup',finish);renderScanner()};
  const down=e=>{start={x:e.clientX,y:e.clientY};window.addEventListener('pointerup',finish,{once:true})};canvas.style.cursor='crosshair';canvas.addEventListener('pointerdown',down,{once:false});
}
